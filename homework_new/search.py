...
def search():
    ...
    if not text:
        ...
        for music in music_list:
            rid=music['rid']
            detailurl = f'http://m.kuwo.cn/newh5/singles/songinfoandlrc?musicId={rid}&httpsStatus=1'
            message = request.get(detailurl).json()['data']['songinfo']
            data={
                'name':message["name"],
        ...
        try:
            with app.app_context():
                ...
                if os.path.exists(file_path):
                    ...
                    musicurl=request.get(infourl).json()['data']['url']
                    musicdata=request.get(musicurl).content
