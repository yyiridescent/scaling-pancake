...
app = Flask(__name__)
url = 'mysql+pymysql://root:123456@localhost:3306/music'  # music
...
class User_History(db.Model):  # 历史记录
    ...
    id = db.Column(db.Integer, db.Foreign('user_message.id'), primary_key=True)
    fav = db.Column(db.Integer, nullable=False, default=0)#default默认值
    history = db.Column(db.Integer, db.Foreign('history.id'), primary_key=True)  # 外键
